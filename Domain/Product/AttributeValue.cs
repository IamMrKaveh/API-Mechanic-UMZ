...
public class AttributeValue : BaseEntity, IAuditable, ISoftDeletable, IActivatable
{
    ...
    // Navigation — استفاده شده در EF Core Configuration
    ...
    {
        if (string.IsNullOrWhiteSpace(value)) throw new DomainException("Value is required.");
        if (string.IsNullOrWhiteSpace(displayValue)) throw new DomainException("Display value is required.");
