...
public class AdminProductService : IAdminProductService
{
    ...
    private readonly IAppLogger<AdminProductService> _logger;
    ...
    public AdminProductService(
        ...
        IAppLogger<AdminProductService> logger,
    ...
    public async Task<ServiceResult<PagedResultDto<AdminProductListDto>>> GetProductsAsync(
        ...
        int pageSize)
    {
        var (products, totalItems) = await _productRepository.GetAdminProductsAsync(
            searchTerm,
            categoryId,
            isActive,
            includeDeleted,
        ...
        var productDtos = products.Select(p => new AdminProductListDto
        {
            ...
            IconUrl = p.Images.FirstOrDefault(i => i.IsPrimary)?.FilePath,
    ...
    private async Task<(bool IsValid, string? ErrorMessage)> ValidateVariantAttributesAsync(List<CreateProductVariantDto> variants)
    {
        ...
        var existingAttributeValues = await _attributeRepository.GetValuesByIdsAsync(allAttributeValueIds);
    ...
    private async Task UpdateVariantsWithStockAsync(Product product, List<CreateProductVariantDto> variantDtos, int userId)
    {
        ...
        foreach (var dto in variantDtos)
        {
            ...
            if (dto.Id > 0)
            {
                ...
                // Sync shipping methods (removed direct context access)
                // This logic should ideally be inside the entity or a repo method, 
                // but since we are refactoring services, we must rely on loaded object graph.
            ...
            var attributeValues = await _attributeRepository.GetValuesByIdsAsync(dto.AttributeValueIds);
            foreach (var attrValue in attributeValues)
            {
                variant.VariantAttributes.Add(new ProductVariantAttribute { AttributeValue = attrValue });
            }
