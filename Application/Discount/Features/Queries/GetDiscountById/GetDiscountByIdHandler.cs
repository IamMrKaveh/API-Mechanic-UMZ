...
public class GetDiscountByIdHandler : IRequestHandler<GetDiscountByIdQuery, ServiceResult<DiscountCodeDetailDto?>>
{
    ...
    public async Task<ServiceResult<DiscountCodeDetailDto?>> Handle(GetDiscountByIdQuery request, CancellationToken cancellationToken)
    {
        var discount = await _discountRepository.GetByIdWithDetailsAsync(request.Id, cancellationToken);
