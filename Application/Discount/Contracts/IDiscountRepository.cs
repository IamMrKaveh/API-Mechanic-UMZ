...
public interface IDiscountRepository
{
    ...
    /// </summary>
    Task<DiscountCode?> GetByIdWithUsagesAsync(int id, CancellationToken ct = default);
    ...
    /// </summary>
    Task<int> CountUserUsageAsync(int discountId, int userId, CancellationToken ct = default);

    /// <summary>
    /// دریافت کدهای تخفیف فعال
    /// </summary>
    Task<IEnumerable<DiscountCode>> GetActiveDiscountsAsync(CancellationToken ct = default);

    /// <summary>
    /// دریافت کدهای تخفیف در حال انقضا
    /// </summary>
    Task<IEnumerable<DiscountCode>> GetExpiringDiscountsAsync(DateTime beforeDate, CancellationToken ct = default);

    /// <summary>
    /// دریافت استفاده بر اساس سفارش
    /// </summary>
    Task<DiscountUsage?> GetUsageByOrderIdAsync(int orderId, CancellationToken ct = default);

    /// <summary>
    /// افزودن کد تخفیف جدید
    /// </summary>
    Task AddAsync(DiscountCode discount, CancellationToken ct = default);
    ...
    /// </summary>
    void SetOriginalRowVersion(DiscountCode entity, byte[] rowVersion);
}