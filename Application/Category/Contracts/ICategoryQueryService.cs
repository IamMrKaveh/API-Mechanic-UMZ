...
public interface ICategoryQueryService
{
    ...
    Task<CategoryWithBrandsDto?> GetCategoryWithBrandsAsync(int categoryId, CancellationToken ct = default);
    ...
    Task<BrandDetailDto?> GetBrandDetailAsync(int groupId, CancellationToken ct = default);

    Task<PaginatedResult<BrandListItemDto>> GetBrandsPagedAsync(
        int? categoryId, string? search, bool? isActive, bool includeDeleted, int page, int pageSize, CancellationToken ct = default);
}