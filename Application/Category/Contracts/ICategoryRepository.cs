...
public interface ICategoryRepository
{
    ...
    /// </summary>
    Task<Domain.Category.Category?> GetByIdWithGroupsAsync(int id, CancellationToken ct = default);

    /// <summary>
    /// دریافت Category به همراه گروه‌ها و محصولات (برای بررسی امکان حذف)
    /// </summary>
    Task<Domain.Category.Category?> GetByIdWithGroupsAndProductsAsync(int id, CancellationToken ct = default);

    /// <summary>
    /// دریافت چندین Category بر اساس شناسه (برای Reorder)
    /// </summary>
    Task<IReadOnlyList<Domain.Category.Category>> GetByIdsAsync(IEnumerable<int> ids, CancellationToken ct = default);

    /// <summary>
    /// دریافت تمام دسته‌بندی‌های فعال و غیر حذف‌شده (برای Reorder)
    /// </summary>
    Task<IReadOnlyList<Domain.Category.Category>> GetAllActiveAsync(CancellationToken ct = default);
    ...
    Task AddAsync(Domain.Category.Category category, CancellationToken ct = default);

    void Update(Domain.Category.Category category);

    void SetOriginalRowVersion(Domain.Category.Category entity, byte[] rowVersion);
}