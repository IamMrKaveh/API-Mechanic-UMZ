...
public sealed class UpdateProductStatsOnReviewRestoredHandler(
    IReviewQueryService reviewQueryService,
    IProductRepository productRepository,
    IUnitOfWork unitOfWork,
    ILogger<UpdateProductStatsOnReviewRestoredHandler> logger)
    : INotificationHandler<DomainEventNotification<ReviewRestoredEvent>>
{
    public async Task Handle(
        DomainEventNotification<ReviewRestoredEvent> notification,
        CancellationToken ct)
    {
        ...
        {
            logger.LogWarning(
                "Cannot recalculate stats. Product {ProductId} not found for restored review {ReviewId}.",
                evt.ProductId.Value,
                evt.ReviewId.Value,
                ct);
            return;
