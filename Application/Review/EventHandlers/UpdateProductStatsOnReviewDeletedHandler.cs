...
public sealed class UpdateProductStatsOnReviewDeletedHandler(
    IReviewQueryService reviewQueryService,
    IProductRepository productRepository,
    IUnitOfWork unitOfWork,
    ILogger<UpdateProductStatsOnReviewDeletedHandler> logger)
    : INotificationHandler<DomainEventNotification<ReviewDeletedEvent>>
{
    public async Task Handle(
        DomainEventNotification<ReviewDeletedEvent> notification,
        CancellationToken ct)
    {
        ...
        {
            logger.LogWarning(
                "Cannot recalculate stats. Product {ProductId} not found for deleted review {ReviewId}.",
                evt.ProductId.Value,
                evt.ReviewId.Value,
                ct);
            return;
