...
public sealed class UpdateProductStatsOnReviewRejectedHandler(
    IReviewQueryService reviewQueryService,
    IProductRepository productRepository,
    IUnitOfWork unitOfWork,
    ILogger<UpdateProductStatsOnReviewRejectedHandler> logger)
    : INotificationHandler<DomainEventNotification<ReviewRejectedEvent>>
{
    public async Task Handle(
        DomainEventNotification<ReviewRejectedEvent> notification,
        CancellationToken ct)
    {
        ...
        {
            logger.LogWarning(
                "Cannot recalculate stats. Product {ProductId} not found for rejected review {ReviewId}.",
                evt.ProductId.Value,
                evt.ReviewId.Value,
                ct);
            return;
