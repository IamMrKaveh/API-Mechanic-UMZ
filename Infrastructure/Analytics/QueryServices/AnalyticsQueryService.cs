...
public sealed class AnalyticsQueryService : IAnalyticsQueryService
{
        ...
        DateTime? fromDate, DateTime? toDate, CancellationToken cancellationToken = default)
    {
        ...
        // Get category data through product -> categoryGroup -> category chain
        var salesData = await _context.OrderItems
            ...
            .Join(_context.Products.IgnoreQueryFilters().Where(p => !p.IsDeleted),
                oi => oi.ProductId,
                p => p.Id,
                (oi, p) => new { oi, p.CategoryGroupId })
            .Join(_context.CategoryGroups.IgnoreQueryFilters().Where(cg => !cg.IsDeleted),
                x => x.CategoryGroupId,
                cg => cg.Id,
        ...
        var categoryInfo = await _context.Categories
            ...
            .Select(c => new
            {
                c.Id,
                Name = c.Name,
                TotalGroups = c.CategoryGroups.Count(cg => !cg.IsDeleted),
                TotalProducts = c.CategoryGroups
    ...
    public async Task<InventoryReportDto> GetInventoryReportAsync(CancellationToken cancellationToken = default)
    {
        var variants = await _context.ProductVariants
            ...
            .Select(v => new
            {
                ...
                v.SellingPrice,
                v.UpdatedAt,
                CategoryGroupId = v.Product != null ? v.Product.CategoryGroupId : 0
            })
            .ToListAsync(cancellationToken);
        ...
        // Category breakdown
        var categoryGroupIds = variants.Select(v => v.CategoryGroupId).Distinct().ToList();
        var categoryMapping = await _context.CategoryGroups
            .IgnoreQueryFilters()
            .Where(cg => !cg.IsDeleted && categoryGroupIds.Contains(cg.Id))
            .Join(_context.Categories.IgnoreQueryFilters().Where(c => !c.IsDeleted),
        ...
        var byCategoryData = variants
            .Where(v => !v.IsUnlimited && categoryLookup.ContainsKey(v.CategoryGroupId))
            .GroupBy(v => categoryLookup[v.CategoryGroupId])
