...
public class LedkaContext : DbContext
{
    ...
    public DbSet<Domain.Security.RateLimit> RateLimits { get; set; } = null!;
    ...
    private void ConfigureSupport(ModelBuilder builder)
    {
        ...
        builder.Entity<TicketMessage>(entity =>
        {
            ...
            entity.HasOne(d => d.Ticket).WithMany(p => p.Messages).HasForeignKey(d => d.TicketId).OnDelete(DeleteBehavior.Cascade);
        });
    }
