...
public class WalletFraudAlertIdConverterTests
{
    private static object CreateConverter()
    {
        var type = typeof(DBContext).Assembly.GetType(
            "Infrastructure.Wallet.Converters.WalletFraudAlertIdConverter");
    ...
    public void Converter_DerivesFromStronglyTypedIdConverter()
    {
        ...
        baseType.GetGenericArguments()[0].ShouldBe(typeof(WalletFraudAlertId));
    }

    [Fact]
    public void Converter_MapsBetweenWalletFraudAlertIdAndGuid()
    {
        dynamic converter = CreateConverter();

        ((Type)converter.ProviderClrType).ShouldBe(typeof(Guid));
        ((Type)converter.ModelClrType).ShouldBe(typeof(WalletFraudAlertId));
    }
    ...
    {
        var converter = CreateConverter();
        var id = WalletFraudAlertId.NewId();
    ...
    public void ConvertFromProvider_RestoresStronglyTypedId()
    {
        ...
        fromProvider.DynamicInvoke(value).ShouldBe(WalletFraudAlertId.From(value));
    }
    ...
    {
        var converter = CreateConverter();
        var original = WalletFraudAlertId.NewId();
        var toProvider = ToProvider(converter).Compile();
        var fromProvider = FromProvider(converter).Compile();
    ...
    {
        WalletFraudAlertId.NewId().ShouldNotBe(WalletFraudAlertId.NewId());
    }
}
