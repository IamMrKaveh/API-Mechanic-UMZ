...
public class WalletLedgerEntryIdConverterTests
{
    private static object CreateConverter()
    {
        var type = typeof(DBContext).Assembly.GetType(
            "Infrastructure.Wallet.Converters.WalletLedgerEntryIdConverter");
    ...
    public void Converter_DerivesFromStronglyTypedIdConverter()
    {
        ...
        baseType.GetGenericArguments()[0].ShouldBe(typeof(WalletLedgerEntryId));
    }

    [Fact]
    public void Converter_MapsBetweenWalletLedgerEntryIdAndGuid()
    {
        dynamic converter = CreateConverter();

        ((Type)converter.ProviderClrType).ShouldBe(typeof(Guid));
        ((Type)converter.ModelClrType).ShouldBe(typeof(WalletLedgerEntryId));
    }
    ...
    {
        var converter = CreateConverter();
        var id = WalletLedgerEntryId.NewId();
    ...
    public void ConvertFromProvider_RestoresStronglyTypedId()
    {
        ...
        fromProvider.DynamicInvoke(value).ShouldBe(WalletLedgerEntryId.From(value));
    }
    ...
    {
        var converter = CreateConverter();
        var original = WalletLedgerEntryId.NewId();
        var toProvider = ToProvider(converter).Compile();
        var fromProvider = FromProvider(converter).Compile();
    ...
    {
        WalletLedgerEntryId.NewId().ShouldNotBe(WalletLedgerEntryId.NewId());
    }
}
