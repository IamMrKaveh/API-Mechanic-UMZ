...
namespace Tests.Infrastructure.Discount.Converters;

public class DiscountRestrictionIdConverterTests
{
    private static object CreateConverter()
    {
        var type = typeof(DBContext).Assembly.GetType(
            "Infrastructure.Discount.Converters.DiscountRestrictionIdConverter");
    ...
    public void Converter_DerivesFromStronglyTypedIdConverter()
    {
        ...
        baseType.GetGenericArguments()[0].ShouldBe(typeof(DiscountRestrictionId));
    }

    [Fact]
    public void Converter_MapsBetweenDiscountRestrictionIdAndGuid()
    {
        dynamic converter = CreateConverter();

        ((Type)converter.ProviderClrType).ShouldBe(typeof(Guid));
        ((Type)converter.ModelClrType).ShouldBe(typeof(DiscountRestrictionId));
    }
    ...
    public void ConvertFromProvider_RestoresStronglyTypedId()
    {
        ...
        fromProvider.DynamicInvoke(value).ShouldBe(DiscountRestrictionId.From(value));
    }
    ...
    {
        var converter = CreateConverter();
        var original = DiscountRestrictionId.NewId();
        var toProvider = ToProvider(converter).Compile();
        var fromProvider = FromProvider(converter).Compile();
