...
public class VariantShippingIdConverterTests
{
    private static object CreateConverter()
    {
        var type = typeof(DBContext).Assembly.GetType(
            "Infrastructure.Variant.Converters.VariantShippingIdConverter");
    ...
    public void Converter_DerivesFromStronglyTypedIdConverter()
    {
        ...
        baseType.GetGenericArguments()[0].ShouldBe(typeof(VariantShippingId));
    }

    [Fact]
    public void Converter_MapsBetweenVariantShippingIdAndGuid()
    {
        dynamic converter = CreateConverter();

        ((Type)converter.ProviderClrType).ShouldBe(typeof(Guid));
        ((Type)converter.ModelClrType).ShouldBe(typeof(VariantShippingId));
    }
    ...
    {
        var converter = CreateConverter();
        var id = VariantShippingId.NewId();
    ...
    public void ConvertFromProvider_RestoresStronglyTypedId()
    {
        ...
        fromProvider.DynamicInvoke(value).ShouldBe(VariantShippingId.From(value));
    }
    ...
    {
        var converter = CreateConverter();
        var original = VariantShippingId.NewId();
        var toProvider = ToProvider(converter).Compile();
        var fromProvider = FromProvider(converter).Compile();
    ...
    {
        VariantShippingId.NewId().ShouldNotBe(VariantShippingId.NewId());
    }
}
