...
namespace Tests.Infrastructure.Review.Converters;

public class ReviewIdConverterTests
{
    private static object CreateConverter()
    {
        var type = typeof(DBContext).Assembly.GetType(
            "Infrastructure.Review.Converters.ReviewIdConverter");
    ...
    public void Converter_DerivesFromStronglyTypedIdConverter()
    {
        ...
        baseType.GetGenericArguments()[0].ShouldBe(typeof(ReviewId));
    }

    [Fact]
    public void Converter_MapsBetweenReviewIdAndGuid()
    {
        dynamic converter = CreateConverter();

        ((Type)converter.ProviderClrType).ShouldBe(typeof(Guid));
        ((Type)converter.ModelClrType).ShouldBe(typeof(ReviewId));
    }
    ...
    public void ConvertFromProvider_RestoresStronglyTypedId()
    {
        ...
        fromProvider.DynamicInvoke(value).ShouldBe(ReviewId.From(value));
    }
    ...
    {
        ReviewId.NewId().ShouldNotBe(ReviewId.NewId());
    }
}
