...
namespace Tests.Infrastructure.Wishlist.Converters;

public class WishlistIdConverterTests
{
    private static object CreateConverter()
    {
        var type = typeof(DBContext).Assembly.GetType(
            "Infrastructure.Wishlist.Converters.WishlistIdConverter");
    ...
    public void Converter_DerivesFromStronglyTypedIdConverter()
    {
        ...
        baseType.GetGenericArguments()[0].ShouldBe(typeof(WishlistId));
    }

    [Fact]
    public void Converter_MapsBetweenWishlistIdAndGuid()
    {
        dynamic converter = CreateConverter();

        ((Type)converter.ProviderClrType).ShouldBe(typeof(Guid));
        ((Type)converter.ModelClrType).ShouldBe(typeof(WishlistId));
    }
    ...
    public void ConvertFromProvider_RestoresStronglyTypedId()
    {
        ...
        fromProvider.DynamicInvoke(value).ShouldBe(WishlistId.From(value));
    }
    ...
    {
        WishlistId.NewId().ShouldNotBe(WishlistId.NewId());
    }
}
