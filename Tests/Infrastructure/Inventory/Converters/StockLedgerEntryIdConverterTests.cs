...
namespace Tests.Infrastructure.Inventory.Converters;

public class StockLedgerEntryIdConverterTests
{
    private static object CreateConverter()
    {
        var type = typeof(DBContext).Assembly.GetType(
            "Infrastructure.Inventory.Converters.StockLedgerEntryIdConverter");
    ...
    public void Converter_DerivesFromStronglyTypedIdConverter()
    {
        ...
        baseType.GetGenericArguments()[0].ShouldBe(typeof(StockLedgerEntryId));
    }

    [Fact]
    public void Converter_MapsBetweenStockLedgerEntryIdAndGuid()
    {
        dynamic converter = CreateConverter();

        ((Type)converter.ProviderClrType).ShouldBe(typeof(Guid));
        ((Type)converter.ModelClrType).ShouldBe(typeof(StockLedgerEntryId));
    }
    ...
    {
        var converter = CreateConverter();
        var id = StockLedgerEntryId.NewId();
    ...
    public void ConvertFromProvider_RestoresStronglyTypedId()
    {
        ...
        fromProvider.DynamicInvoke(value).ShouldBe(StockLedgerEntryId.From(value));
    }
    ...
    {
        var converter = CreateConverter();
        var original = StockLedgerEntryId.NewId();
        var toProvider = ToProvider(converter).Compile();
        var fromProvider = FromProvider(converter).Compile();
    ...
    [Fact]
    public void ConvertFromProvider_WithEmptyGuid_Throws()
    {
        var converter = CreateConverter();
        var fromProvider = FromProvider(converter).Compile();

        Should.Throw<TargetInvocationException>(() => fromProvider.DynamicInvoke(Guid.Empty));
    }
}
