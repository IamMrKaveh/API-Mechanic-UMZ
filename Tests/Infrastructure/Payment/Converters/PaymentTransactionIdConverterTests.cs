...
namespace Tests.Infrastructure.Payment.Converters;

public class PaymentTransactionIdConverterTests
{
    private static object CreateConverter()
    {
        var type = typeof(DBContext).Assembly.GetType(
            "Infrastructure.Payment.Converters.PaymentTransactionIdConverter");
    ...
    public void Converter_DerivesFromStronglyTypedIdConverter()
    {
        ...
        baseType.GetGenericArguments()[0].ShouldBe(typeof(PaymentTransactionId));
    }

    [Fact]
    public void Converter_MapsBetweenPaymentTransactionIdAndGuid()
    {
        dynamic converter = CreateConverter();

        ((Type)converter.ProviderClrType).ShouldBe(typeof(Guid));
        ((Type)converter.ModelClrType).ShouldBe(typeof(PaymentTransactionId));
    }
    ...
    public void ConvertFromProvider_RestoresStronglyTypedId()
    {
        ...
        fromProvider.DynamicInvoke(value).ShouldBe(PaymentTransactionId.From(value));
    }
    ...
    {
        var converter = CreateConverter();
        var original = PaymentTransactionId.NewId();
        var toProvider = ToProvider(converter).Compile();
        var fromProvider = FromProvider(converter).Compile();
