...
namespace Tests.Infrastructure.Product.Converters;

public class ProductIdConverterTests
{
    private static object CreateConverter()
    {
        var type = typeof(DBContext).Assembly.GetType(
            "Infrastructure.Product.Converters.ProductIdConverter");
    ...
    [Fact]
    public void Converter_MapsBetweenProductIdAndGuid()
    {
        dynamic converter = CreateConverter();

        ((Type)converter.ProviderClrType).ShouldBe(typeof(Guid));
        ((Type)converter.ModelClrType).ShouldBe(typeof(ProductId));
    }
    ...
    public void ConvertFromProvider_RestoresStronglyTypedId()
    {
        ...
        fromProvider.DynamicInvoke(value).ShouldBe(ProductId.From(value));
    }
    ...
    {
        ProductId.NewId().ShouldNotBe(ProductId.NewId());
    }
}
