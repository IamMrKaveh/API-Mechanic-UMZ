...
namespace Tests.Infrastructure.User.Converters;

public class UserAddressIdConverterTests
{
    private static object CreateConverter()
    {
        var type = typeof(DBContext).Assembly.GetType(
            "Infrastructure.User.Converters.UserAddressIdConverter");
    ...
    public void Converter_DerivesFromStronglyTypedIdConverter()
    {
        ...
        baseType.GetGenericArguments()[0].ShouldBe(typeof(UserAddressId));
    }

    [Fact]
    public void Converter_MapsBetweenUserAddressIdAndGuid()
    {
        dynamic converter = CreateConverter();

        ((Type)converter.ProviderClrType).ShouldBe(typeof(Guid));
        ((Type)converter.ModelClrType).ShouldBe(typeof(UserAddressId));
    }
    ...
    public void ConvertFromProvider_RestoresStronglyTypedId()
    {
        ...
        fromProvider.DynamicInvoke(value).ShouldBe(UserAddressId.From(value));
    }
    ...
    {
        var converter = CreateConverter();
        var original = UserAddressId.NewId();
        var toProvider = ToProvider(converter).Compile();
        var fromProvider = FromProvider(converter).Compile();
    ...
    {
        UserAddressId.NewId().ShouldNotBe(UserAddressId.NewId());
    }
}
