...
namespace Tests.Infrastructure.Security.Converters;

public class OtpIdConverterTests
{
    private static object CreateConverter()
    {
        var type = typeof(DBContext).Assembly.GetType(
            "Infrastructure.Security.Converters.OtpIdConverter");
    ...
    public void Converter_DerivesFromStronglyTypedIdConverter()
    {
        ...
        baseType.GetGenericArguments()[0].ShouldBe(typeof(OtpId));
    }

    [Fact]
    public void Converter_MapsBetweenOtpIdAndGuid()
    {
        dynamic converter = CreateConverter();

        ((Type)converter.ProviderClrType).ShouldBe(typeof(Guid));
        ((Type)converter.ModelClrType).ShouldBe(typeof(OtpId));
    }
    ...
    public void ConvertFromProvider_RestoresStronglyTypedId()
    {
        ...
        fromProvider.DynamicInvoke(value).ShouldBe(OtpId.From(value));
    }
    ...
    {
        OtpId.NewId().ShouldNotBe(OtpId.NewId());
    }
}
